...
io.on('connection', function(socket) {
  socket.on('disconnect', function() {
    const id = socketIdToPlayerId[socket.id];
    if(id === undefined || id === null)
      return;
    delete socketIdToPlayerId[socket.id];
    players[id].Connected = false;
    // TODO: Emit player disconnected
  });

  /////////////////////////////////////////////////////////////////////////
  // Creates a new player
  socket.on("createPlayer", function(callback){
      ...
      players[id] = {Id: id, Connected: true};
      console.log(players);
      callback({Id: id});
  });
  ...
  socket.on("getPlayer", function(data, callback){
    const id = socketIdToPlayerId[socket.id];
    if(id !== undefined && id !== null)
      players[id].Connected = false;
    ...
    if(player === undefined || player === null) {
      callback("Invalid player id");
      return;
    }
    players[data.PlayerId].Connected = true;
    console.log(players);
    callback(players[data.PlayerId]);
  });
